...
class MarketViewModel(private val repo: MarketRepository): ViewModel() {
    ...
    fun fetchMarketDetail(
        token: String, idMarket: String, idUser: Int, privateKey: PrivateKey
    ) = liveData<Resource<*>>(viewModelScope.coroutineContext + Dispatchers.Main) {
        emit(Resource.Loading<Unit>())

        try {
            emit( Resource.Success<CreditMarketClient>( repo.fetchMarketDetail(
                token, idMarket, idUser, privateKey
            )))
        } catch (e: HttpException) {
            if (e.code() == 401 || e.code() == 403) {
                emit(Resource.Failure(e))
